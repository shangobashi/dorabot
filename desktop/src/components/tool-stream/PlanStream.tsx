...
const TOOL_META: Record<string, { icon: typeof LayoutGrid; verb: string; color: string }> = {
  projects_view: { icon: Eye, verb: "viewing", color: "text-violet-400" },
  projects_add: { icon: Plus, verb: "adding", color: "text-blue-400" },
  projects_update: { icon: Pencil, verb: "updating", color: "text-amber-400" },
  projects_delete: { icon: Trash2, verb: "deleting", color: "text-red-400" },
...
export function PlanStream({ name, input, output, isError, streaming }: ToolUIProps) {
  ...
  const meta = TOOL_META[name] || TOOL_META.projects_view
    ...
    <div className="rounded-lg overflow-hidden border border-border/60 bg-[var(--stream-base)]">
      <div className="flex items-center gap-3 px-3 py-3">
        ...
        <div className="flex-1 min-w-0 space-y-1.5">
          ...
          {/* view filter */}
          {(name === "projects_view" || name === "tasks_view") && (parsed.status || parsed.goalId) && (
            <motion.div
